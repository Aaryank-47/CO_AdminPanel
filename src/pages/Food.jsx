...
const Food = () => {
  ...
  // --- DATA FETCHING AND LOGIC (Your functions remain here) ---
  ...
  const Modal = ({ isOpen, onClose, title, children, onConfirm, confirmText }) => {
     ...
     return (
        ...
        <div className="bg-white rounded-2xl shadow-xl w-full max-w-md m-4">
            ...
            <button type="button" onClick={onClose} className="px-4 py-2 border border-slate-300 rounded-lg text-sm font-medium text-slate-700 hover:bg-slate-100">Cancel</button>
            <button type="button" onClick={onConfirm} className="px-4 py-2 bg-slate-900 text-white rounded-lg text-sm font-medium hover:bg-slate-800">{confirmText}</button>
            </div>
        </div>
        </div>
     );
 ...
 const SkeletonLoader = () => (
    <div className="animate-pulse">
      <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-2 gap-6 mb-8">
        {[...Array(4)].map((_, i) => (
          <div key={i} className="bg-white p-6 rounded-xl shadow-sm h-32">
      ...
      <div className="grid grid-cols-1 lg:grid-cols-5 gap-6 mb-8">
        <div className="lg:col-span-2 bg-white p-6 rounded-xl shadow-sm h-64"></div>
        <div className="lg:col-span-3 bg-white p-6 rounded-xl shadow-sm h-64"></div>
      </div>
      ...
      <div className="max-w-7xl mx-auto">
        <header className="flex flex-col sm:flex-row justify-between sm:items-center mb-8">
          <div>
            ...
            <p className="text-slate-500 mt-1">Oversee your entire food catalog in one place.</p>
          </div>
          <div className="flex space-x-3 mt-4 sm:mt-0">
            <button onClick={handleRefresh} className="px-4 py-2 bg-white text-slate-700 rounded-lg shadow-sm hover:bg-slate-50 transition-colors flex items-center text-sm font-medium border border-slate-200">Refresh</button>
            <button onClick={() => setIsAddModalOpen(true)} className="px-4 py-2 bg-slate-900 text-white rounded-lg shadow-sm hover:bg-slate-800 transition-colors flex items-center text-sm font-medium">Add Item</button>
          </div>
        </header>
          ...
          <>
            {foodItems.length === 0 ? (
              <div className="text-center py-16 bg-white rounded-xl shadow-sm border border-slate-200">
                <Player autoplay loop src={emptyAnimation} style={{ height: '200px', width: '200px', margin: 'auto' }} />
                <h3 className="mt-4 text-xl font-semibold text-slate-800">Your Menu is Empty</h3>
                <p className="mt-2 text-slate-500">Add your first food item to get started.</p>
                <button onClick={() => setIsAddModalOpen(true)} className="mt-6 px-5 py-2.5 bg-slate-900 text-white rounded-lg shadow-sm hover:bg-slate-800 transition-colors font-medium">Add Food Item</button>
              </div>
            ) : (
              <>
                <section className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-6 mb-8">
                  <div className="bg-white p-6 rounded-xl shadow-sm border border-slate-200"><h3 className="text-sm font-medium text-slate-500">Total Items</h3><p className="text-3xl font-bold text-slate-800 mt-2">{menuStats.totalItems}</p></div>
                  <div className="bg-white p-6 rounded-xl shadow-sm border border-slate-200"><h3 className="text-sm font-medium text-slate-500">Active Items</h3><p className="text-3xl font-bold text-green-600 mt-2">{menuStats.activeCount}</p></div>
                  <div className="bg-white p-6 rounded-xl shadow-sm border border-slate-200"><h3 className="text-sm font-medium text-slate-500">Inactive Items</h3><p className="text-3xl font-bold text-red-600 mt-2">{menuStats.inactiveCount}</p></div>
                  <div className="bg-white p-6 rounded-xl shadow-sm border border-slate-200"><h3 className="text-sm font-medium text-slate-500">Total Menu Value</h3><p className="text-3xl font-bold text-slate-800 mt-2">₹{menuStats.totalValue.toFixed(2)}</p></div>
                </section>

                <section className="grid grid-cols-1 lg:grid-cols-5 gap-6 mb-8">
                  <div className="lg:col-span-2 bg-white p-6 rounded-xl shadow-sm border border-slate-200"><h3 className="text-lg font-semibold text-slate-800 mb-4">Item Availability</h3><div className="h-64 flex items-center justify-center"><Doughnut data={availabilityChartData} options={{ responsive: true, maintainAspectRatio: false, plugins: { legend: { position: 'bottom' }}}} /></div></div>
                  <div className="lg:col-span-3 bg-white p-6 rounded-xl shadow-sm border border-slate-200"><h3 className="text-lg font-semibold text-slate-800 mb-4">Price Distribution</h3><div className="h-64"><Bar data={priceChartData} options={{ responsive: true, maintainAspectRatio: false, plugins: { legend: { display: false }}, scales: { y: { beginAtZero: true }}}} /></div></div>
                </section>
                      ...
                      <tbody className="bg-white divide-y divide-slate-200">
                        {foodItems.map((item) => (
                          <tr key={item._id} className={!item.isActive ? "bg-slate-50 opacity-60" : "hover:bg-slate-50"}>
                            <td className="px-6 py-4"><img src={item.image || "https://via.placeholder.com/150"} alt={item.name} className="h-12 w-12 rounded-lg object-cover" /></td>
                            ...
                            <td className="px-6 py-4"><span className={`px-3 py-1 rounded-full text-xs font-medium ${item.isActive ? "bg-green-100 text-green-800" : "bg-red-100 text-red-800"}`}>{item.isActive ? "Active" : "Inactive"}</span></td>
                            <td className="px-6 py-4 text-sm font-medium space-x-4">
                              <button onClick={() => { setIsEditModalOpen(true); setCurrentFood({...item, foodName: item.name, foodPrice: item.price}); }} className="">Edit</button>
                              <button onClick={() => handleDeleteFood(item._id)} className="">Delete</button>
                            </td>
                          </tr>
                        ))}
                      </tbody>
