...
const Order = () => {
  ...
  const [past30daysOrder, setPast30daysOrder] = useState(null);
                ...
                <tr key={rowIndex}>
                  {[...Array(7)].map((_, cellIndex) => (
                    <td key={cellIndex} className="px-6 py-4 whitespace-nowrap">
                      <div className={`h-4 bg-gray-200 dark:bg-gray-700 rounded animate-pulse ${cellIndex === 2 ? 'w-3/4' : cellIndex === 6 ? 'w-1/2' : ''}`}></div>
                    </td>
                  ))}
                </tr>
              ))}
  ...
  useEffect(() => {
    const fetchOrders = async () => {
      try {
        ...
        // const response = await fetch(`http://localhost:5000/api/v1/orders/get-all-orders/${adminId}`, {
        const response = await fetch(`https://canteen-order-backend.onrender.com/api/v1/orders/get-all-orders/${adminId}`, {
          method: 'GET',
          headers: {
  ...
  useEffect(() => {
    ...
    const fetchLast30DaysOrders = async () => {
      ...
      try {
        ...
        const data = await response.json();
        if (!data.success) {
          toast.error("Failed to load last 30 days orders");
          return;
        }
        setPast30daysOrder(data.orders);
  ...
  const fetchOrdersPerDayData = async () => {
    try {
      ...
      const data = await response.json();
      if (response.ok) {
        const labels = data.ordersPerDay.map(item => {
      ...
      toast.error("Failed to load order trends data");
    } finally {
      setChartLoading(false);
    }
  };
  ...
  const updateOrderStatus = async (orderId, newStatus) => {
    try {
      ...
      // const response = await fetch(`localhost:5000/api/v1/orders/admin-order-update/${orderId}`,
      const response = await fetch(`https://canteen-order-backend.onrender.com/api/v1/orders/admin-order-update/${orderId}`,
        {
          method: 'PUT',
          headers: {
            'Authorization': `Bearer ${adminToken}`,
  ...
  const dailyData = ordersPerDayData || {
  ...
  const currentOrders = activeTab === "month"
    ? (past30daysOrder || [])
    : (orders.today || []);
        ...
        <div className="grid grid-cols-1 lg:grid-cols-2 gap-5 mb-8">
          <div className={`p-5 rounded-xl ${darkMode ? 'bg-gray-800' : 'bg-white'} shadow`}>
            <div className="flex justify-between items-center mb-4">
              <h3 className={`text-lg font-semibold ${darkMode ? 'text-white' : 'text-gray-900'}`}>Daily Orders</h3>
              {chartLoading && (
            ...
            <div className="h-64">
              {chartLoading ? (
                <div className="flex items-center justify-center h-full">
                  <div className={`animate-spin rounded-full h-8 w-8 border-t-2 ${darkMode ? 'border-purple-500' : 'border-purple-600'}`}></div>
                </div>
              ) : (
